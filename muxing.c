...
int main(int argc, char **argv) {
	...
	do {
		...
		while (1) {
			...
			frame_index++;
			packet.pts = av_rescale_q_rnd(packet.pts, input_format_context->streams[0]->time_base, output_format_context->streams[0]->time_base, AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX);
			packet.dts = av_rescale_q_rnd(packet.dts, input_format_context->streams[0]->time_base, output_format_context->streams[0]->time_base, AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX);
